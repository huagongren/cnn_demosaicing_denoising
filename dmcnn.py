...
def testing_model(mymodel, img_path, id):
    ...
    mosaic = mosaicing_CFA_Bayer(myimg)
    ...
    if mosaic.shape[1] % 33 != 0:
        for i in range(33, mosaic.shape[0], 33):
            ...
            image_patches.append(patch)
            bilin_patches.append(bilin[i-33:i, mosaic.shape[1]-33:mosaic.shape[1], :])
    ...
    if mosaic.shape[0] % 33 != 0:
        for j in range(33, mosaic.shape[1], 33):
            ...
            image_patches.append(patch)
            bilin_patches.append(bilin[mosaic.shape[0]-33:mosaic.shape[0], j-33:j, :])
    ...
    if mosaic.shape[0] % 33 != 0 and mosaic.shape[1] % 33 != 0:
        ...
        image_patches.append(patch)
        bilin_patches.append(bilin[mosaic.shape[0]-33:mosaic.shape[0], mosaic.shape[1]-33:mosaic.shape[1], :])
    ...
    with torch.no_grad():
        for i in range(0, tensor_patches.shape[0]):
            input_patch = tensor_patches[i:i+1024, :, :, :]
    ...
    showprocess(myimg, mosaic, bilin, demosaiced, id)
...
#加载原有模型
weights_path = 'weights/d'
if not os.path.exists(weights_path):
    os.mkdir(weights_path)
checkpoint_path = 'dmcnn_vd_21.weight'
...
if flag.lower() == "y":
    # 执行train操作
    dataset = ImagePatchDataset("picture", patch_size=(33, 33))
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        load_checkpoint(os.path.join(weights_path, f'dmcnn_vd_{epoch}.weight'), model_vd, optimizer_vd, loss, epoch=epoch)
        plt.show()
